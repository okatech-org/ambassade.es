...
const categoryLabels: Record<string, { label: string; color: string }> = {
  ...
  communique: { label: 'Communiqué', color: 'bg-orange-500/10 text-orange-600' },
}
...
function AdminPostsPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(t('admin.posts.deleteError', 'Erreur lors de la suppression'))
    }
  }
        ...
        <CardContent>
          {isLoading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t('admin.posts.columns.title', 'Titre')}</TableHead>
                  <TableHead>{t('admin.posts.columns.category', 'Catégorie')}</TableHead>
                  <TableHead>{t('admin.posts.columns.status', 'Statut')}</TableHead>
              ...
              <TableBody>
                {posts?.map((post) => {
                  const cat = categoryLabels[post.category] || { label: post.category, color: 'bg-gray-500/10 text-gray-600' }
                  return (
                    <TableRow key={post._id}>
                      ...
                      <TableCell>
                        <Badge variant={post.status === 'published' ? 'default' : 'outline'}>
                          {post.status === 'published' ? t('admin.posts.published', 'Publié') : t('admin.posts.draft', 'Brouillon')}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        {post.publishedAt 
                      ...
                      <TableCell className="text-right space-x-1">
                        ...
                        <Button variant="ghost" size="icon" asChild>
                          <Link to="/admin/posts/$postId/edit" params={{ postId: post._id }}>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('admin.posts.deleteConfirmDesc', 'Cette action est irréversible. L\'article sera définitivement supprimé.')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
