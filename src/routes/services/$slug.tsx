...
function ServiceDetailPage() {
	...
	const config =
		categoryConfig[localizedService.category] ||
	...
	const serviceName = localizedService.title;
	const serviceDescription = localizedService.description;
			...
			<div className="flex-1">
				...
				<section className="relative overflow-hidden py-16 px-6">
					...
					<div className="max-w-4xl mx-auto relative z-10">
						...
						<div className="flex flex-col md:flex-row md:items-start gap-8">
							<div
								className={`p-6 rounded-3xl ${config.bgColor} ${config.color} shadow-lg backdrop-blur-sm ring-1 ring-inset ring-white/10 shrink-0`}
							>
							...
							<div className="flex-1 pt-2">
								<Badge
									variant="secondary"
									className={`${config.bgColor} ${config.color} mb-4 border-0 px-3 py-1`}
								>
									{categoryLabel}
								</Badge>
				...
				<section className="py-8 px-6 pb-24">
					<div className="max-w-4xl mx-auto space-y-8">
								...
								<div>
									...
									<p className="text-muted-foreground leading-relaxed">
										{t(
											...
											"Les services Passeport et Visa sont de l'autorité de l'Antenne DGDI. Veuillez noter que ce n'est pas le Consulat Général qui établit les Passeports et les Visas en France.",
										)}
									</p>
								</div>
							</div>
						...
						{/* Required Documents */}
						{localizedService.requirements &&
							localizedService.requirements.length > 0 && (
								<div className="glass-card rounded-3xl p-8 md:p-10 space-y-6">
									<div className="flex items-center gap-3 pb-4 border-b border-border/40">
									...
									<ul className="grid sm:grid-cols-2 gap-4">
										{localizedService.requirements.map((doc, index) => (
											<li
												key={index}
												className="flex items-start gap-3 p-4 rounded-xl bg-background/40 border border-border/40 hover:border-primary/20 transition-colors"
											>
						...
						{localizedService.delay && (
							<div className="glass-card rounded-3xl p-8 md:p-10 space-y-6">
								<div className="flex items-center gap-3 pb-4 border-b border-border/40">
									...
									<h2 className="text-xl font-bold text-foreground">
										{t("services.practicalInfo", "Informations pratiques")}
									</h2>
								</div>
