...
export function ServiceCard({
	...
	return (
		<div
			className={`glass-card p-6 h-full flex flex-col group hover:border-primary/30 transition-all duration-300 ${isUrgent ? "border-red-500/30 hover:border-red-500/50" : ""} ${greenHighlight ? "bg-emerald-50/60 dark:bg-emerald-950/20 border-emerald-200/50 dark:border-emerald-800/30 hover:border-emerald-400/50" : ""}`}
		>
			{/* Header with icon and badges */}
			<div className="flex items-start justify-between mb-4">
				<div
					className={`w-12 h-12 rounded-[14px] flex items-center justify-center ${color}`}
				>
			...
			{validity && (
				<div className="flex flex-wrap gap-2 mb-4">
					<div className="flex items-center gap-1.5 text-xs text-muted-foreground">
						...
						<span>
							{t("serviceCard.validity", "Validité")} : {validity}
						</span>
					</div>
				</div>
			)}
			...
			<div className="mt-auto space-y-4 pt-4 border-t border-border/30">
				...
				<div className="grid grid-cols-2 gap-3">
					...
					>
						📋 {t("serviceCard.viewDetails", "Voir la fiche")}
					</button>
					<a
					...
					>
						{t("serviceCard.startProcess", "Faire la démarche")}
					</a>
				</div>
			</div>
		</div>
